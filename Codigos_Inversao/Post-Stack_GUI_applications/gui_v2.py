...
class Funcs():    
    ...
    def input_tied_wvlt(self):
        ...
        if wav_files:
            ...
            times = []
            for file in wav_files:
                arr = pd.read_csv(file, sep='\s+')
                wavs.append(arr.values[:,0] / np.max(abs(arr.values[:,0])))
                times.append(np.arange(0, int(arr.columns[2])*(len(arr.values[:,0])), int(arr.columns[2])))
